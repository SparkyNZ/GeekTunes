...
//    std::fstream myIn( fileName,createAtrr);
  ...
  std::fstream myIn( fileName, std::fstream::in | std::fstream::out | std::fstream::binary);
...
// ------------------------------------------ Looking for a description file.
                ...
                while (fileNameExtensions[n] != 0)
                {
                    ...
                    if ( MYSTRICMP( (char*) fileName,fileName2.get())!=0 &&
                ...
                if (ret != LOAD_NOT_MINE)
                {
                    ...
                    while (fileNameExtensions[n] != 0)
                    {
                            ...
                            loadFile(fileName2.get(),fileBuf2) )
                        {
                            ...
                            if ( MYSTRICMP ( (char*)fileNameExtensions[n], ".mus")==0 )
                            {
...
bool SidTune::saveC64dataFile( const char* fileName, bool overWriteFlag )
{
    ...
    if ( status )
    {
      ...
      std::ofstream fMyOut( fileName, std::ios::binary | std::ios::out );
